    ...
    public void downloadFile(String filePath, java.io.OutputStream outputStream) {
        try {
            ...
            if (StrUtil.isNotBlank(customDomain)) {
                // 从自定义域名中提取对象名
                // https://oss.youlai.tech/default/20221120/test.jpg → 20221120/websocket.jpg
                objectName = filePath.substring(customDomain.length() + 1 + bucketName.length() + 1);
            } else {
                // 从默认URL中提取对象名
                // http://localhost:9000/default/20221120/test.jpg → 20221120/websocket.jpg
                objectName = filePath.substring(endpoint.length() + 1 + bucketName.length() + 1);
            }
            ...
            try (java.io.InputStream inputStream = minioClient.getObject(getObjectArgs)) {
