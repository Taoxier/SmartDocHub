...
function takeAccessToken() {
    ...
    try {
        ...
        // console.log('返回的令牌:', authObj.token);
...
export function isAdmin() {
    ...
    return userInfo.roles.includes('ROOT') || userInfo.roles.includes('ADMIN');
}
...
function storeAccessToken(remember, token, expire, userId, username, roles) {
    ...
    // console.log('存储的认证信息:', authObj);
    // console.log('存储的认证信息字符串:', str);
    console.log('remember:', remember);
    if (remember) {
        console.log('存储到localStorage');
        localStorage.setItem(authItemName, str)
        // console.log('存储后的localStorage:', localStorage.getItem(authItemName));
    } else {
        console.log('存储到sessionStorage');
        sessionStorage.setItem(authItemName, str)
        // console.log('存储后的sessionStorage:', sessionStorage.getItem(authItemName));
    }
}
...
function login(username, password, remember, success, failure = defaultFailure) {
    ...
    internalPost('/api/v1/auth/login', formData, {
        ...
        ElMessage.success(`登录成功，很高兴再次见到您，${username} `)
        success(data)
    }, failure)
}
...
function unauthorized() {
    const token = takeAccessToken();
    console.log('unauthorized函数返回:', !token);
    return !token
}
